...
def main() -> None:
    ...
    print(f"timing: {datetime.now() - start}, prettier: {formatd(datetime.now() - start)}, finished reading json files", file=sys.stderr)
    CI_passes = dict()
    ...
    print(f"timing: {datetime.now() - start}, prettier: {formatd(datetime.now() - start)}, finished populating dictionaries", file=sys.stderr)

    print_on_the_queue_page(input_data.nondraft_prs, CI_passes, "on_the_queue.html")
    print(f"timing: {datetime.now() - start}, prettier: {formatd(datetime.now() - start)}, finished printing queue page", file=sys.stderr)
    ...
    print(f"timing: {datetime.now() - start}, prettier: {formatd(datetime.now() - start)}, finished printing TOC", file=sys.stderr)
    ...
    print(f"timing: {datetime.now() - start}, prettier: {formatd(datetime.now() - start)}, manually filtered new PRs", file=sys.stderr)

    print(gather_pr_statistics(CI_passes, prs_to_list, input_data.nondraft_prs, input_data.draft_prs))
    print(f"timing: {datetime.now() - start}, prettier: {formatd(datetime.now() - start)}, computed PR stats", file=sys.stderr)
    ...
    print(f"timing: {datetime.now() - start}, prettier: {formatd(datetime.now() - start)}, computed bad titles, unlabelled + contradictory PRs", file=sys.stderr)
    for kind in Dashboard._member_map_.values():
    ...
    print(f"timing: {datetime.now() - start}, prettier: {formatd(datetime.now() - start)}, all dashboards printed", file=sys.stderr)
    print(HTML_FOOTER)
