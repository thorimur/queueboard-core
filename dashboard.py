...
def main() -> None:
    def formatd(delta: timedelta) -> str:
        if delta.days > 0:
            ...
            return f"{delta.hours} hours and {delta.minutes} minutes"
        elif delta.minutes > 0:
            return f"{delta.minutes} minutes and {delta.seconds} seconds"
        else:
            return f"{delta.seconds},{delta.milliseconds} seconds"
    start = datetime.now()
    input_data = read_json_files()
    print(f"timing: {datetime.now() - start}, prettier: {format_delta(datetime.now() - start)}, finished reading json files", file=sys.stderr)
    CI_passes = dict()
    ...
    print(f"timing: {datetime.now() - start}, prettier: {format_delta(datetime.now() - start)}, finished populating dictionaries", file=sys.stderr)

    print_on_the_queue_page(input_data.nondraft_prs, CI_passes, "on_the_queue.html")
    print(f"timing: {datetime.now() - start}, prettier: {format_delta(datetime.now() - start)}, finished printing queue page", file=sys.stderr)
    ...
    print(f"timing: {datetime.now() - start}, prettier: {format_delta(datetime.now() - start)}, finished printing TOC", file=sys.stderr)
    ...
    print(f"timing: {datetime.now() - start}, prettier: {format_delta(datetime.now() - start)}, manually filtered new PRs", file=sys.stderr)

    print(gather_pr_statistics(CI_passes, prs_to_list, input_data.nondraft_prs, input_data.draft_prs))
    print(f"timing: {datetime.now() - start}, prettier: {format_delta(datetime.now() - start)}, computed PR stats", file=sys.stderr)
    ...
    print(f"timing: {datetime.now() - start}, prettier: {format_delta(datetime.now() - start)}, computed bad titles, unlabelled + contradictory PRs", file=sys.stderr)
    for kind in Dashboard._member_map_.values():
    ...
    print(f"timing: {datetime.now() - start}, prettier: {format_delta(datetime.now() - start)}, all dashboards printed", file=sys.stderr)
    print(HTML_FOOTER)
