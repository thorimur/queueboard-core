...
QUERY_QUEUE_BUT_MERGE_CONFLICT=$(prepare_query "sort:updated-asc is:pr state:open -is:draft status:success -label:blocked-by-other-PR -label:awaiting-CI -label:awaiting-author -label:WIP -label:delegated -label:auto-merge-after-CI label:merge-conflict -label:awaiting-zulip -label:please-adopt -label:help-wanted")
